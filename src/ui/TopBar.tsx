...
export const TopBar: React.FC = () => {
  ...
  const badgeBoxClass = isDesign2026
    ? 'bg-[#181C24]/90 border border-[#283244] shadow-lg rounded-2xl text-white px-2 py-1 sm:px-3 sm:py-2'
    : 'hud-parchment rounded-2xl px-2 py-1 sm:px-3 sm:py-2 text-[#3B1F0D]';

  return (
    <header className="fixed top-0 left-0 right-0 z-50 pointer-events-none p-1.5 sm:p-3 select-none">
      <div className="max-w-4xl mx-auto flex items-start justify-between gap-1 sm:gap-3">

        {/* ── LEFT: Telegram Profile & Daily Missions under Level ── */}
        <div className="flex flex-col items-stretch pointer-events-auto gap-1 min-w-[125px] max-w-[150px] sm:min-w-[170px] sm:max-w-none">
          <div 
            onClick={() => {
              sounds.playClick();
            ...
            <div className="relative shrink-0">
              {tgProfile.photoUrl ? (
                <img
                  ...
                  alt={tgProfile.name}
                  className="w-7 h-7 sm:w-8.5 sm:h-8.5 rounded-xl object-cover border border-white/20 shadow"
                />
              ) : (
                <div className="w-7 h-7 sm:w-8.5 sm:h-8.5 rounded-xl bg-gradient-to-tr from-purple-600 to-indigo-500 border border-white/30 flex items-center justify-center font-black text-white text-[10px] sm:text-xs shadow">
                  {tgProfile.name.charAt(0)}
                </div>
              )}
              {/* Online Green Dot */}
              <span className="absolute -bottom-0.5 -right-0.5 w-2 h-2 sm:w-2.5 sm:h-2.5 bg-emerald-500 border-2 border-[#181C24] rounded-full" />
            </div>

            {/* User Name & Level */}
            <div className="flex flex-col flex-1 min-w-0">
              <div className="flex items-center gap-1 justify-between">
                <span className={`font-bold text-[10px] sm:text-xs tracking-tight truncate ${
                ...
                {/* Level Badge */}
                <div className="flex items-center justify-center w-4.5 h-4.5 sm:w-5 sm:h-5 bg-[#2B1B0F] border border-amber-400/80 rounded-md sm:rounded-lg font-black text-[9px] sm:text-[10px] text-yellow-300 shadow shrink-0">
                  {level}
                </div>
              </div>
        ...
        <div className="flex flex-col items-end pointer-events-auto gap-1">
          <div className="flex items-center gap-1 sm:gap-1.5">
            ...
            <div className={`flex items-center gap-1 sm:gap-1.5 ${badgeBoxClass}`}>
              <CoinSvg />
              <span className={`font-extrabold text-[10px] sm:text-xs md:text-sm min-w-[20px] sm:min-w-[32px] ${
            ...
            >
              <WoodSvg />
              <span className={`font-extrabold text-[10px] sm:text-xs md:text-sm ${
            ...
            <div className={`flex items-center gap-1 sm:gap-1.5 pl-1.5 pr-1 py-1 sm:pl-2.5 sm:pr-1.5 sm:py-1.5 ${badgeBoxClass}`}>
              <EnergySvg />
              <span className={`font-extrabold text-[10px] sm:text-xs md:text-sm ${
              ...
              <button
                ...
                className="w-4.5 h-4.5 sm:w-5.5 sm:h-5.5 rounded-md sm:rounded-lg bg-emerald-600 hover:bg-emerald-500 border border-emerald-300 text-white flex items-center justify-center text-[9px] sm:text-xs font-black shadow active:scale-90 transition-transform cursor-pointer shrink-0"
              >
                +
              </button>
            </div>
