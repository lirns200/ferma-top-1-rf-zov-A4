...
export function createMountainWaterfallGroup(season: SeasonType): THREE.Group {
  ...
  const waterPlungeMat = new THREE.MeshStandardMaterial({
  ...
  // ── 1. Granite Canyon Cliffs & Gorge Wings ────────────────────────────
  const cliffGeo = new THREE.DodecahedronGeometry(3.2, 1);

  // Left Canyon Wall (flanking x = -5 to -8)
  const leftCliffs = [
    { x: -5.2, y: 3.5, z: 2.0, scale: [2.0, 3.2, 2.0], rot: 0.4 },
    { x: -6.2, y: 8.0, z: -2.0, scale: [2.4, 3.5, 2.2], rot: -0.2 },
    { x: -6.8, y: 13.0, z: -6.0, scale: [2.6, 3.8, 2.5], rot: 0.6 },
    { x: -4.2, y: 1.8, z: 4.5, scale: [1.8, 2.0, 1.8], rot: 0.1 },
  ];
  leftCliffs.forEach((c, idx) => {
    const mesh = new THREE.Mesh(cliffGeo, idx % 2 === 0 ? rockDarkMat : rockMidMat);
    mesh.position.set(c.x, c.y, c.z);
    mesh.scale.set(c.scale[0], c.scale[1], c.scale[2]);
    mesh.rotation.set(0.1, c.rot, 0.15);
    mesh.castShadow = true;
    mesh.receiveShadow = true;
    group.add(mesh);
  });

  // Right Canyon Wall (flanking x = 5 to 8)
  const rightCliffs = [
    { x: 5.2, y: 3.5, z: 2.0, scale: [2.0, 3.2, 2.0], rot: -0.4 },
    { x: 6.2, y: 8.0, z: -2.0, scale: [2.4, 3.5, 2.2], rot: 0.3 },
    { x: 6.8, y: 13.0, z: -6.0, scale: [2.6, 3.8, 2.5], rot: -0.5 },
    { x: 4.2, y: 1.8, z: 4.5, scale: [1.8, 2.0, 1.8], rot: -0.1 },
  ];
  rightCliffs.forEach((c, idx) => {
    const mesh = new THREE.Mesh(cliffGeo, idx % 2 === 0 ? rockMidMat : rockDarkMat);
    mesh.position.set(c.x, c.y, c.z);
    mesh.scale.set(c.scale[0], c.scale[1], c.scale[2]);
    mesh.rotation.set(-0.1, c.rot, -0.15);
    mesh.castShadow = true;
  ...
  // Stepped Granite Shelves where water plunges
  const shelf1 = new THREE.Mesh(new THREE.BoxGeometry(6.8, 1.2, 3.2), rockDarkMat);
  shelf1.position.set(0, 10.2, -4.5);
  shelf1.castShadow = true;
  shelf1.receiveShadow = true;

  const shelf2 = new THREE.Mesh(new THREE.BoxGeometry(7.4, 1.2, 3.2), rockDarkMat);
  shelf2.position.set(0, 4.8, -1.0);
  shelf2.castShadow = true;
  shelf2.receiveShadow = true;
  group.add(shelf1, shelf2);

  // ── 2. Cascading Waterfall Curtains & Water Sheets ────────────────────
  // Upper Stream / Source Pool (y = 11.2)
  const sourcePool = new THREE.Mesh(new THREE.BoxGeometry(6.2, 0.4, 5.5), waterDeepMat);
  sourcePool.position.set(0, 10.9, -7.0);
  group.add(sourcePool);

  // Tier 1: Upper Plunge (y = 10.9 to 5.8)
  const tier1Water = new THREE.Mesh(new THREE.PlaneGeometry(5.6, 5.6, 8, 8), waterPlungeMat);
  tier1Water.name = 'waterfall_curtain_1';
  tier1Water.position.set(0, 7.9, -3.0);
  tier1Water.rotation.x = -Math.PI / 2 + 0.35; // Sloping plunge
  tier1Water.castShadow = true;

  // Upper Foam Rapids & Crest
  const tier1Foam = new THREE.Mesh(new THREE.BoxGeometry(5.8, 0.35, 1.2), foamMat);
  tier1Foam.name = 'waterfall_foam_1';
  tier1Foam.position.set(0, 10.6, -4.2);
  group.add(tier1Water, tier1Foam);

  // Mid Shelf Pool (y = 5.0)
  const midPool = new THREE.Mesh(new THREE.BoxGeometry(6.6, 0.3, 3.0), waterDeepMat);
  midPool.position.set(0, 5.0, -0.8);
  group.add(midPool);

  // Tier 2: Lower Grand Plunge (y = 5.0 to 0.0)
  const tier2Water = new THREE.Mesh(new THREE.PlaneGeometry(6.4, 5.4, 8, 8), waterPlungeMat);
  tier2Water.name = 'waterfall_curtain_2';
  tier2Water.position.set(0, 2.5, 1.5);
  tier2Water.rotation.x = -Math.PI / 2 + 0.42;
  tier2Water.castShadow = true;

  // Lower Foam Crest
  const tier2Foam = new THREE.Mesh(new THREE.BoxGeometry(6.6, 0.35, 1.2), foamMat);
  tier2Foam.name = 'waterfall_foam_2';
  tier2Foam.position.set(0, 5.0, 0.0);
  group.add(tier2Water, tier2Foam);

  // ── 3. Base Plunge Pool, Foam Rings & River Rapids ────────────────────
  // Plunge Splash Pool at River Head (y = -0.02)
  const splashPool = new THREE.Mesh(new THREE.CylinderGeometry(4.5, 5.0, 0.3, 16), waterDeepMat);
  splashPool.position.set(0, -0.02, 4.0);
  group.add(splashPool);

  // Expanding White Foam Rings at Base
  for (let r = 0; r < 3; r++) {
    const ringGeo = new THREE.RingGeometry(1.3 + r * 1.0, 1.9 + r * 1.0, 16);
    ringGeo.rotateX(-Math.PI / 2);
    const ring = new THREE.Mesh(ringGeo, foamMat);
    ring.name = `waterfall_foam_ring_${r}`;
    ring.position.set(0, 0.02 + r * 0.01, 3.8);
    group.add(ring);
  }

  // Splashing River Rocks at the Waterfall Base
  const splashRockGeo = new THREE.DodecahedronGeometry(0.75, 0);
  [
    { x: -3.0, z: 4.2, s: 1.3 },
    { x: 2.8, z: 4.0, s: 1.2 },
    { x: -1.1, z: 5.8, s: 0.9 },
    ...
    const rock = new THREE.Mesh(splashRockGeo, rockMidMat);
    rock.position.set(sr.x, 0.3, sr.z);
    rock.scale.set(sr.s, sr.s * 0.7, sr.s);
  ...
  [
    { x: -5.8, y: 7.2, z: 0.5, s: 1.2 },
    { x: -7.0, y: 13.0, z: -3.8, s: 1.4 },
    { x: 5.6, y: 7.0, z: 0.0, s: 1.1 },
    { x: 6.8, y: 12.8, z: -4.2, s: 1.3 },
  ].forEach(pt => {
...
export function createAnimalPenGroup(configId: string): THREE.Group {
  ...
  // Fenced Area Ground (Mud for pigs, rich grass for chickens/cows/sheep)
  const isMud = configId === 'pig_pen';
  const groundGeo = new THREE.BoxGeometry(2.85, 0.12, 2.85);
  const groundColor = isMud ? '#542D0C' : '#65A30D';
  const groundMat = getCachedColorMaterial(groundColor, 0.9);
  const ground = new THREE.Mesh(groundGeo, groundMat);
  ground.position.y = 0.06;
  ground.receiveShadow = true;
  group.add(ground);

  // Mud Puddle in pig pen
  if (isMud) {
    const mudGeo = new THREE.CylinderGeometry(0.8, 0.85, 0.04, 10);
    const mudMat = getCachedColorMaterial('#3B1808', 0.3);
    const mud = new THREE.Mesh(mudGeo, mudMat);
    mud.position.set(0.3, 0.13, 0.2);
    group.add(mud);
  }

  // Wooden Fence Posts & Rails around the perimeter
  const fenceMat = getCachedColorMaterial('#78350F', 0.75);
  const postGeo = new THREE.BoxGeometry(0.12, 0.7, 0.12);
  const corners = [
    [-1.35, -1.35], [1.35, -1.35], [-1.35, 1.35], [1.35, 1.35],
    [0, -1.35], [0, 1.35], [-1.35, 0], [1.35, 0]
  ];
  ...
  [0.3, 0.55].forEach(ry => {
    ...
    const rS = new THREE.Mesh(railHGeo, fenceMat);
    rS.position.set(0, ry, 1.35);
    ...
    group.add(rN, rS, rW, rE);
  });

  // Corner Animal Shelter / Hen House
  if (configId === 'chicken_coop') {
    // Cute elevated wooden hen house with perches and ramp
    const coopGeo = new THREE.BoxGeometry(1.1, 0.9, 0.9);
    const coopMat = getCachedColorMaterial('#DC2626', 0.65);
    const coop = new THREE.Mesh(coopGeo, coopMat);
    coop.position.set(-0.7, 0.6, -0.7);
    coop.castShadow = true;
    group.add(coop);

    const coopRoofGeo = new THREE.ConeGeometry(0.85, 0.55, 4);
    const coopRoof = new THREE.Mesh(coopRoofGeo, getCachedColorMaterial('#78350F', 0.6));
    coopRoof.position.set(-0.7, 1.3, -0.7);
    coopRoof.rotation.y = Math.PI / 4;
    group.add(coopRoof);

    // Wooden Ramp
    const rampGeo = new THREE.BoxGeometry(0.3, 0.04, 0.7);
    const ramp = new THREE.Mesh(rampGeo, getCachedColorMaterial('#B45309', 0.8));
    ramp.position.set(-0.7, 0.25, -0.05);
    ramp.rotation.x = 0.55;
    group.add(ramp);

    // Straw Nesting Box with eggs
    const nestGeo = new THREE.CylinderGeometry(0.25, 0.2, 0.1, 8);
    const nestMat = getCachedColorMaterial('#FACC15', 0.8);
    const nest = new THREE.Mesh(nestGeo, nestMat);
    nest.position.set(0.65, 0.15, -0.65);
    const egg = new THREE.Mesh(new THREE.SphereGeometry(0.06, 6, 6), getCachedColorMaterial('#FFFFFF', 0.3));
    egg.position.set(0.65, 0.22, -0.65);
    group.add(nest, egg);

  } else if (configId === 'cow_pasture') {
    // Water trough and hay rack
    const troughGeo = new THREE.BoxGeometry(0.9, 0.3, 0.4);
    const trough = new THREE.Mesh(troughGeo, getCachedColorMaterial('#78350F', 0.8));
    trough.position.set(-0.65, 0.2, -0.7);
    const waterGeo = new THREE.BoxGeometry(0.8, 0.08, 0.3);
    const water = new THREE.Mesh(waterGeo, getCachedColorMaterial('#38BDF8', 0.2));
    water.position.set(-0.65, 0.3, -0.7);
    group.add(trough, water);

    // Hay Feeder
    const hayRack = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.5, 0.3), getCachedColorMaterial('#FACC15', 0.8));
    hayRack.position.set(0.65, 0.3, -0.7);
    group.add(hayRack);

  } else if (configId === 'pig_pen') {
    // Wooden Pig Shelter Hut
    const hutGeo = new THREE.BoxGeometry(1.1, 0.8, 0.9);
    const hut = new THREE.Mesh(hutGeo, getCachedColorMaterial('#9A3412', 0.8));
    hut.position.set(-0.7, 0.5, -0.7);
    hut.castShadow = true;
    group.add(hut);

    // Feed Trough with apples/carrots
    const trough = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.22, 0.35), getCachedColorMaterial('#78350F', 0.8));
    trough.position.set(0.6, 0.18, 0.6);
    group.add(trough);

  } else {
    // Sheep Shelter
    const shelter = new THREE.Mesh(new THREE.BoxGeometry(1.2, 0.9, 1.0), getCachedColorMaterial('#B45309', 0.7));
    shelter.position.set(-0.65, 0.55, -0.65);
...
export function createDecorationMesh(configId: string): THREE.Group {
  ...
  if (configId.includes('fence')) {
    ...
    // Cobblestone stone path with organic stepping stones
    const isStone = configId.includes('stone');
    const pathMat = getCachedColorMaterial(isStone ? '#94A3B8' : '#B45309', 0.9);

    if (isStone) {
      [
        [-0.22, -0.22, 0.38], [0.22, -0.22, 0.34], [-0.2, 0.22, 0.35], [0.24, 0.22, 0.4]
      ].forEach(([px, pz, sz]) => {
        const stone = new THREE.Mesh(new THREE.CylinderGeometry(sz / 2, sz / 2, 0.04, 6), pathMat);
        stone.position.set(px, 0.03, pz);
        stone.receiveShadow = true;
        group.add(stone);
      });
    } else {
      const pathGeo = new THREE.BoxGeometry(0.92, 0.03, 0.92);
